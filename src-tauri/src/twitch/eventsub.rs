...
use super::{get_token, helix, TwitchState};
...
#[tauri::command]
pub(crate) async fn add_chat_channel(
    app: tauri::AppHandle,
    broadcaster_id: String,
    is_mod: bool,
) -> Result<(), String> {
...
#[tauri::command]
pub(crate) async fn remove_chat_channel(
    app: tauri::AppHandle,
    broadcaster_id: String,
) -> Result<(), String> {
    let state = app.state::<TwitchState>();
    let tx_guard = state.eventsub_tx.lock().unwrap();
    if let Some(tx) = tx_guard.as_ref() {
        tx.send(EventSubCmd::Remove { broadcaster_id })
            .map_err(|e| e.to_string())?;
    }
    Ok(())
}
...
async fn ensure_chat_task(app: &tauri::AppHandle) -> Result<(), String> {
    {
        let state = app.state::<TwitchState>();
        let guard = state.eventsub_tx.lock().unwrap();
        if guard.is_some() {
            return Ok(());
        }
    }
    ...
    let state = app.state::<TwitchState>();
    let mut guard = state.eventsub_tx.lock().unwrap();
    ...
    *guard = Some(tx);
    ...
    tauri::async_runtime::spawn(async move {
        ...
        let state = app_clone.state::<TwitchState>();
        *state.eventsub_tx.lock().unwrap() = None;
    });
    Ok(())
}
        ...
        loop {
            tokio::select! {
                cmd = cmd_rx.recv() => match cmd {
                    Some(EventSubCmd::Add { broadcaster_id, is_mod }) => {
                    ...
                    Some(EventSubCmd::Remove { broadcaster_id }) => {
...
async fn subscribe_to_chat(
    ...
    if let Some(id) = create(app, helix, token, broadcaster_id, "channel.chat.message",
        ChannelChatMessageV1::new(broadcaster_id, user_id), transport.clone()).await { ids.push(id); }
    if let Some(id) = create(app, helix, token, broadcaster_id, "channel.chat.notification",
        ChannelChatNotificationV1::new(broadcaster_id, user_id), transport.clone()).await { ids.push(id); }
    if let Some(id) = create(app, helix, token, broadcaster_id, "channel.chat.message_delete",
        ChannelChatMessageDeleteV1::new(broadcaster_id, user_id), transport.clone()).await { ids.push(id); }
    if let Some(id) = create(app, helix, token, broadcaster_id, "channel.chat.clear",
        ChannelChatClearV1::new(broadcaster_id, user_id), transport.clone()).await { ids.push(id); }
    if let Some(id) = create(app, helix, token, broadcaster_id, "channel.chat.clear_user_messages",
        ChannelChatClearUserMessagesV1::new(broadcaster_id, user_id), transport.clone()).await { ids.push(id); }

    if is_mod {
        if let Some(id) = create(app, helix, token, broadcaster_id, "channel.shoutout.create",
            ChannelShoutoutCreateV1::new(broadcaster_id, user_id), transport.clone()).await { ids.push(id); }
        if let Some(id) = create(app, helix, token, broadcaster_id, "channel.follow",
            ChannelFollowV2::new(broadcaster_id, user_id), transport.clone()).await { ids.push(id); }
        if let Some(id) = create(app, helix, token, broadcaster_id, "channel.moderate",
