...
export default function StreamTooltip(props: Props) {
  return (
    <Portal>
      <div
        style={{ position: "fixed", left: `${props.x}px`, top: `${props.y}px`, transform: "translateY(-50%)" }}
      ...
      >
        <p class="text-text text-sm font-semibold whitespace-nowrap truncate">
          ...
          <Show when={props.channel.game_name}>
            <span class="text-text-muted font-normal"> - {props.channel.game_name}</span>
          </Show>
        </p>
        <Show when={props.channel.title}>
          <p class="text-text-muted text-xs mt-0.5 break-words">{props.channel.title}</p>
        </Show>
